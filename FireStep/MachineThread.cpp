...
size_t MachineThread::readEEPROM(uint8_t *eeprom_addr, char *dst, size_t maxLen) {
    ...
    uint8_t c = machine.pDuino->eeprom_read_byte(eeprom_addr);
    if (!dst || (c != '{' && c != '[')) {
        return 0;
    }
    ...
    for (len=0; len<maxLen-2; len++) {
        c = machine.pDuino->eeprom_read_byte(eeprom_addr+len);
        if (c == 255 || c == 0) {
            break;
        }
        dst[len] = c;
...
Status MachineThread::syncConfig() {
    ...
    *out++ = '\n';
    ...
    uint8_t *eepAddr = 0;
    machine.pDuino->eeprom_write_byte(eepAddr, ' '); // disable eeprom
    for (int16_t i=1; i<=len; i++) { // include terminator
        machine.pDuino->eeprom_write_byte(eepAddr+i, buf[i]);
    ...
    if (status == STATUS_BUSY_PARSED) {
        ...
        machine.pDuino->eeprom_write_byte(eepAddr, buf[0]); // enable eeprom
        machine.pDuino->eeprom_write_byte(eepAddr+len-1, 0); // remove EOL
