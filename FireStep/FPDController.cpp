...
Status FPDController::processDimension(JsonCommand& jcmd, JsonObject& jobj, const char* key) {
    ...
    if (strcmp_PS(OP_dim, key) == 0) {
        ...
        if ((s = jobj[key]) && *s == 0) { // query
            ...
            status = jobj[key].success() ? status : STATUS_FIELD_ERROR;
        } else {
            value = jobj[key];
            jobj[key].set(value, 5);
        }
        ...
        status = processField<PH5TYPE, PH5TYPE>(jobj, key, value);
		if (value != machine.delta.getGearRatio(DELTA_AXIS_1)) {
        ...
        status = processField<PH5TYPE, PH5TYPE>(jobj, key, value);
		if (value != machine.delta.getGearRatio(DELTA_AXIS_2)) {
        ...
        status = processField<PH5TYPE, PH5TYPE>(jobj, key, value);
		if (value != machine.delta.getGearRatio(DELTA_AXIS_3)) {
