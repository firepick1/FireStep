...
Status Machine::processAxis(JCommand &jcmd, JsonObject& jobj, const char* key, char group) {
	...
	if (strcmp("tm",key)==0 || strcmp("tm",key+1)==0) {
		if ((s=jobj[key]) && *s==0) { // query
			status = (jobj[key] = (double) axis[iAxis].travelMax).success() ? status : STATUS_ERROR;
		} else {
			axis[iAxis].travelMax = (float)(double) jobj["tm"];
		}
	}
	return status;
}
